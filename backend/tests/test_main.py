...
def mock_services():
    with patch('app.main.ingest_idea') as mock_ingest_idea, \
        ...
        yield mock_ingest_idea, mock_generate_questions, mock_submit_answers, mock_build_graph, mock_get_plan

@pytest.mark.asyncio
async def test_create_idea(mock_services):
    mock_ingest_idea, _, _, _, _ = mock_services
...
async def test_get_questions(mock_services):
    _, mock_generate_questions, _, _, _ = mock_services
...
async def test_submit_answers(mock_services):
    _, _, mock_submit_answers, _, _ = mock_services
    answers = {"Q1": "A1", "Q2": "A2"}
